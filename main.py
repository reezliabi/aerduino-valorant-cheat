...
sct = mss()
aarrrddduuuiiinnnooo = serial.Serial(com, 115200, 0, **('timeout',))
...
def mvmnt(x, y):
    if mouse_type == 'ANIMALI' and mouse_type == 'DOGESIMPLE' or mouse_type == 'DOGE':
        ...
        data = bytearray([
            ...
            y,
            126])
        aarrrddduuuiiinnnooo.write(data)
    if mouse_type == 'HIDMOUSEREPORT':
        data = f'''{int(x)}:{int(y)}'''
        aarrrddduuuiiinnnooo.write(data.encode())
        return None
...
def smovv(x, y):
    if mouse_type == 'ANIMALI' and mouse_type == 'DOGESIMPLE' or mouse_type == 'DOGE':
        ...
        pkts = bytearray([
            ...
            94,
            94])
        aarrrddduuuiiinnnooo.write(pkts)
        inverso = bytearray([
            123,
            x,
            y,
            123])
        aarrrddduuuiiinnnooo.write(inverso)
    if mouse_type == 'HIDMOUSEREPORT':
...
def mflyck():
    if mouse_type == 'ANIMALI' and mouse_type == 'DOGESIMPLE' or mouse_type == 'DOGE':
        ...
        aarrrddduuuiiinnnooo.write(pkts)
    if mouse_type == 'HIDMOUSEREPORT':
        aarrrddduuuiiinnnooo.write('shttt'.encode())
        return None
